...
namespace DVRK {
    ...
    public class URDFJoint : MonoBehaviour
    {
        ...
        public void SetJawValue(float val) {
            if (rb == null)
            {
                rb = transform.GetChild(0).GetComponent<Rigidbody>();
            ...
            if (independent) {
                switch (jointType) {
                    case JointType.Continuous:
                        rb.MoveRotation(jointObject.transform.parent.rotation * Quaternion.Euler(jointOperator * val));
                        currentJointValue = val;
                        break;
                    ...
                    case JointType.Revolute:
                        if (val <= jointLimit.y + delta && val >= jointLimit.x - delta) {
                            ...
                            rb.MoveRotation(jointObject.transform.parent.rotation * Quaternion.Euler(jointOperator * jointLimit.y));
                            currentJointValue = jointLimit.y;
                        }
                        else {
                            rb.MoveRotation(jointObject.transform.parent.rotation * Quaternion.Euler(jointOperator * jointLimit.x));
                            currentJointValue = jointLimit.x;
                        }
                        break;
                    ...
                    case JointType.Prismatic:
                        if (val <= jointLimit.y + delta && val >= jointLimit.x - delta) {
                            Vector3 targetWorld = jointObject.transform.parent.TransformPoint(jointOperator * val);
                            rb.MovePosition(targetWorld);
                            currentJointValue = val;
                        }
                        else if (val > jointLimit.y + delta) {
                            Vector3 targetWorld = jointObject.transform.parent.TransformPoint(jointOperator * jointLimit.y);
                            rb.MovePosition(targetWorld);
                            currentJointValue = jointLimit.y;
                        }
                        else {
                            Vector3 targetWorld = jointObject.transform.parent.TransformPoint(jointOperator * jointLimit.x);
                            rb.MovePosition(targetWorld);
                            currentJointValue = jointLimit.x;
                        }
                        break;
                ...
                foreach (URDFJoint j in mimicJoints) {
                    j.SetJawValue(currentJointValue);
                }
            }
            // mimic joints
            else {
